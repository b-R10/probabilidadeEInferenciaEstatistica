...
tabela <- table(dados$`Região de procedencia`)

freq_simples_acumulada <- matrix(c(cumsum(tabela)))

freq_relativa <- matrix(c(100 * table(dados$`Região de procedencia`) / sum(table(dados$`Região de procedencia`))))
...
barplot(table(dados$`Região de procedencia`),
        main = "Distribuicao - Regiao de procedencia",
        ylab = "Frequencia",
        xlab = "Regiao",
